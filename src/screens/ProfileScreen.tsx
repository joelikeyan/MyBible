...
export function ProfileScreen() {
  ...
  const handleToggleRecording = async () => {
    if (isRecording) {
      const result = await voiceCloningService.stopRecording();
      ...
      const started = await voiceCloningService.startRecording((duration) => {
  ...
  const handleCreateVoice = async () => {
    ...
    const result = await voiceCloningService.createVoiceClone(
      recordedUri,
      `My Voice ${new Date().toLocaleDateString()}`
    );
    ...
    if (result.success) {
      ...
      alert('Failed to create voice clone: ' + result.error);
    }
              ...
              {!recordedUri ? (
                <>
                  ...
                  <Text style={styles.recordTimer}>
                    {isRecording ? voiceCloningService.formatDuration(recordingDuration) : 'Tap to start'}
                  </Text>
                </>
              ) : (
                <View style={styles.postRecordActions}>
                  <TouchableOpacity
                    style={styles.actionButton}
                    onPress={() => voiceCloningService.playPreview(recordedUri)}
                  >
                    <Ionicons name="play" size={20} color="#6B4EFF" />
                    <Text style={styles.actionButtonText}>Play Preview</Text>
                  </TouchableOpacity>
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <>
                        ...
                        <Text style={[styles.actionButtonText, { color: '#FFFFFF' }]}>Create Voice</Text>
                      </>
                    )}
                  </TouchableOpacity>
